...
def generateNeurone(scale, loc=defaultGaussianMean):
    V = {
        ...
        'threshold': getFromGaussian(getSd(7.07070707070707e-07), 10e-3) if varyVThreshold else 10e-3,
    ...
    R = {
        'm': getFromGaussian(getSd(511.7056856187291), 1e6) if varyRm else 1e6  # R_m - Ohms (Ω)

    }
    if varyC:
        # capacitance - s/Ω
        C = getFromGaussian(getSd(1.294314381270903e-12), 10e-3/R['m'])
        T = {
            # tau_m - seconds
            'm': R['m']*C
        }
