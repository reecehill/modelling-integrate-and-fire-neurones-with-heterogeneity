...
for coEfficientOfVariationIndex, coEfficientOfVariation in enumerate(p.coEfficientOfVariations):
    ...
    if(len(p.coEfficientOfVariations) <= 2):
        ...
        plt.plot(p.timeSteps*1e3, currentToInject*1e9, color='green')
        plt.ylabel('$I_{ext}$ (nA)', color='green')
        plt.tight_layout()
...
plt.figure()
if p.varyRm:
    neuronalPopulationIndicesSortedByVariedProperty = np.argsort(
        np.array([n['R']['m'] for n in neuronalPopulation]), axis=0)
if p.varyVThreshold:
    neuronalPopulationIndicesSortedByVariedProperty = np.argsort(
        np.array([n['V']['threshold'] for n in neuronalPopulation]), axis=0)
if p.varyC:
    neuronalPopulationIndicesSortedByVariedProperty = np.argsort(
        np.array([n['C'] for n in neuronalPopulation]), axis=0)
if p.varyTm:
    neuronalPopulationIndicesSortedByVariedProperty = np.argsort(
        np.array([n['T']['m'] for n in neuronalPopulation]), axis=0)
neuronalPopulationOrderedByIncreasingVariedProperty = np.take_along_axis(np.array(
    neuronalPopulation), indices=neuronalPopulationIndicesSortedByVariedProperty, axis=0)
if p.varyRm:
    vectorOfVariedProperty = np.multiply([n['R']['m'] for n in neuronalPopulationOrderedByIncreasingVariedProperty], 1e-6)
if p.varyVThreshold:
    vectorOfVariedProperty = np.multiply([n['V']['threshold'] for n in neuronalPopulationOrderedByIncreasingVariedProperty], 10e3)
if p.varyC:
    vectorOfVariedProperty = [n['C'] for n in neuronalPopulationOrderedByIncreasingVariedProperty]
if p.varyTm:
    vectorOfVariedProperty = np.multiply(
        [n['T']['m'] for n in neuronalPopulationOrderedByIncreasingVariedProperty], 10e3)
    
colorMap = mpl.colors.LinearSegmentedColormap.from_list('colorMap', [
                                                        'blue', 'red'])
# Create gradient from lowest Rm to highest Rm
levels = np.linspace(np.min(vectorOfVariedProperty), np.max(vectorOfVariedProperty), 100)
...
spikeEventPositionsForPopulation = [np.array(n['isSpikingAtTime']).nonzero()[
    0] for n in neuronalPopulationOrderedByIncreasingVariedProperty]
spikeEventColorsForPopulation = [
    colorMap(variedProperty) for variedProperty in normaliseData(vectorOfVariedProperty)]
...
colorbar = plt.colorbar(
    CS3, ticks=[np.min(vectorOfVariedProperty), (np.max(vectorOfVariedProperty)/2),  np.max(vectorOfVariedProperty)], label='$R_m$ (MΩ)')

for i in list(range(5)):
    plt.figure()
    color = 'tab:red'
    neurone = neuronalPopulation[i]
    plt.suptitle('Time course of $I_{ext}$ and a neurone\'s $V_m$')
    plt.title('$R_m$: '+str(neurone['R']['m']*1e-6)+'MΩ, $V_{thr}$: '+str(
        neurone['V']['threshold']*1e3) + 'mV, $C_m$:  '+str(neurone['C']*1e3))
    plt.plot(p.timeSteps*1e3, neurone['V']['m']*1e3, color=color)
